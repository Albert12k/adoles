...
export async function exportLeadershipReport() {
  ...
  const activityMax = Math.max(1, report.studies, report.approvedAttendance, report.quizCorrect, report.activities);
  const activityBars = [['Resumos', report.studies, '#16504D'], ['Presenças', report.approvedAttendance, '#E7A93D'], ['Acertos', report.quizCorrect, '#E8683F'], ['Atividades', report.activities, '#7769A8']].map(([label, value, color]) => `<div class="bar-row"><span>${label}</span><div class="track"><div class="bar" style="width:${Math.round(Number(value) / activityMax * 100)}%;background:${color}"></div></div><b>${value}</b></div>`).join('');
  const classMax = Math.max(1, ...report.classes.map(item => item.points));
  const classBars = report.classes.slice(0, 10).map(item => `<div class="bar-row"><span>${escapeHtml(item.className)}</span><div class="track"><div class="bar" style="width:${Math.round(item.points / classMax * 100)}%;background:#16504D"></div></div><b>${item.points}</b></div>`).join('');
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4;margin:18mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#152420;margin:0;font-size:11px}.hero{background:#0F3535;color:white;border-radius:18px;padding:26px;margin-bottom:22px}.brand{color:#E7A93D;font-size:12px;font-weight:800;letter-spacing:2px}.hero h1{font-size:28px;margin:8px 0 4px}.hero p{color:#D9E8E4;margin:0}.section-title{font-size:16px;color:#0F3535;margin:24px 0 12px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:9px}.card{border:1px solid #DCE5DF;border-radius:12px;padding:13px;min-height:70px}.value{font-size:22px;font-weight:800;color:#16504D}.label{font-size:9px;color:#60706A;margin-top:5px}.charts{display:grid;grid-template-columns:1fr 1fr;gap:18px}.chart{border:1px solid #DCE5DF;border-radius:14px;padding:14px}.chart h3{font-size:11px;margin:0 0 12px}.bar-row{display:grid;grid-template-columns:72px 1fr 34px;gap:7px;align-items:center;margin:8px 0;font-size:8px}.track{height:9px;background:#EEF2ED;border-radius:6px;overflow:hidden}.bar{height:100%;border-radius:6px}.bar-row b{text-align:right}table{width:100%;border-collapse:collapse;font-size:8px}thead{display:table-header-group}th{background:#16504D;color:white;padding:8px 5px;text-align:left}td{padding:8px 5px;border-bottom:1px solid #DCE5DF}tr:nth-child(even){background:#F4F7F4}.privacy{margin-top:20px;background:#EEF2ED;border-left:4px solid #E7A93D;padding:12px;color:#60706A}.footer{margin-top:22px;padding-top:12px;border-top:1px solid #DCE5DF;color:#60706A;font-size:8px;display:flex;justify-content:space-between}.detail{break-before:auto}</style></head><body><div class="hero"><div class="brand">VIVA IASD</div><h1>Relatório de liderança</h1><p>${escapeHtml(report.scopeLabel)} · dados consolidados de períodos oficialmente encerrados</p></div><h2 class="section-title">Resumo geral</h2><div class="grid">${cards}</div><h2 class="section-title">Gráficos consolidados</h2><div class="charts"><div class="chart"><h3>Participação por categoria</h3>${activityBars}</div><div class="chart"><h3>Pontos por base · Top 10</h3>${classBars || '<span>Sem dados</span>'}</div></div><div class="detail"><h2 class="section-title">Detalhamento por base</h2><table><thead><tr><th>#</th><th>Base</th><th>Membros</th><th>Resumos</th><th>Presenças</th><th>Acertos</th><th>Atividades</th><th>Pontos</th><th>Fechamentos</th></tr></thead><tbody>${rows || '<tr><td colspan="9">Nenhuma base disponível.</td></tr>'}</tbody></table></div><div class="privacy"><b>Privacidade:</b> este documento apresenta somente dados consolidados. Textos de resumos, respostas e avaliações individuais não são incluídos.</div><div class="footer"><span>Gerado em ${escapeHtml(new Date(report.generatedAt).toLocaleString('pt-BR'))}</span><span>VIVA IASD · Ministério de Adolescentes</span></div></body></html>`;
