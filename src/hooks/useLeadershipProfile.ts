  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (role === 'admin') {
        ...
        if (active) setState({ name: profile.name ?? 'Administrador', scope: 'Visão geral do projeto', metrics: [[String(districts.data().count), 'distritos'], [String(classes.data().count), 'classes'], [String(members.data().count), 'membros']], managedClasses: [] });
