...
export type ApprovalType = 'attendance' | 'challenge' | 'roleRequest' | 'classJoinRequest' | 'studyRecord' | 'quizAttempt';
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (type === 'attendance' || type === 'studyRecord' || type === 'quizAttempt') {
        ...
        const ids = directed.docs.map(item => item.id);
        if (!ids.length) return;
        approvalsQuery = query(collection(db!, 'classJoinRequests'), where('classId', 'in', ids), where('status', '==', 'pending'), limit(30));
      ...
      unsubscribe = onSnapshot(approvalsQuery, snapshot => setItems(snapshot.docs.map(item => {
        ...
        return { id: item.id, name: data.name ?? data.title ?? data.userName ?? 'Adolescente', copy: type === 'quizAttempt' ? 'Resposta do quiz aguardando correção' : type === 'studyRecord' ? String(data.summary ?? 'Resumo enviado') : type === 'attendance' ? `Semana ${data.week} · aguardando presença` : type === 'classJoinRequest' ? 'Solicitação para entrar na classe' : type === 'challenge' ? `${data.bonusPoints ?? 0} pontos · desafio mensal` : `Pedido para ${data.requestedRole === 'director' ? 'diretor' : 'coordenador'}` };
      })));
    })();
