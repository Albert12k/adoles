...
function ManagementDetail({ title, role, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; onBack: () => void }) {
  ...
  useEffect(() => { if (isRisk) listClassEngagement().then(result => { setEngagementClassId(result.classId); setEngagementMembers(result.members); }).catch(() => undefined); }, [isRisk]);
  ...
  const saveManagement = async () => {
    ...
    try {
      ...
      if (firebaseEnabled && isPresenceTheme) { await updatePresenceScenario(selectedScenario); setMemberNotice(selectedScenario === 'auto' ? 'Rotação automática ativada para a base' : 'Cenário aplicado para toda a base'); }
      setSaved(true);
