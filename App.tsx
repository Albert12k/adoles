...
import { cancelEventAttendance, confirmEventAttendance, createDistrictEvent, listCurrentDistrictEvents, listDistrictEvents, listMyEventRsvps, type DistrictEvent } from './src/services/events';
...
function StudyScreen({ classId, userName }: { classId: string; userName: string }) {
  ...
  const [studyHistory, setStudyHistory] = useState<Array<{ id: string; source?: 'lesson' | 'bible' | 'book'; passage?: string; summary?: string; score?: number; feedbackVisible?: boolean; feedback?: string; createdAt?: { toDate?: () => Date } }>>([]);
...
function ProfileScreen({ name, className, classId, districtId, ageGroup, initialStatus, initialThemeColor, onExit }: { name: string; className: string; classId: string; districtId: string; ageGroup: string; initialStatus: string; initialThemeColor: string; onExit: () => Promise<void> }) {
  ...
  useEffect(() => { listMyEventRsvps().then(setConfirmedEvents).catch(() => undefined); }, []);
  ...
  if (communityView !== 'hub') {
    ...
    return (
      <View style={styles.pagePad}>
        ...
        {communityView === 'eventos' && <>{events.length === 0 && <Text style={styles.pageIntro}>Nenhum encontro publicado para seu distrito.</Text>}{events.map(event => { const confirmed = confirmedEvents.includes(event.id); return <View key={event.id} style={styles.formCard}><Text style={styles.manageTitle}>{event.title}</Text><Text style={styles.manageCopy}>{event.dateLabel} · {event.location}</Text><Pressable style={[styles.approveButton, confirmed && styles.approveButtonDone]} onPress={async () => { if (confirmed) { await cancelEventAttendance(event); setConfirmedEvents(items => items.filter(id => id !== event.id)); } else { await confirmEventAttendance(event); setConfirmedEvents(items => [...items, event.id]); } }}><Text style={[styles.approveButtonText, confirmed && styles.approveButtonTextDone]}>{confirmed ? '✓ Confirmado · toque para cancelar' : 'Confirmar participação'}</Text></Pressable></View>; })}</>}
...
function ManagementDetail({ title, role, selectedClassId, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; selectedClassId?: string; onBack: () => void }) {
  ...
  const approveItem = async (item: { id: string; name: string }) => {
    ...
    setActionError('');
    try { await reviewLeadershipItem(approvalType, item.id, true, approvalType === 'studyRecord' ? { evaluation: studyEvaluations[item.id] ?? 'good', feedback: studyFeedbacks[item.id] } : undefined); toggleApproval(item.name); }
  ...
  const rejectItem = async (item: { id: string; name: string }) => {
    ...
    setActionError('');
    try { await reviewLeadershipItem(approvalType, item.id, false, approvalType === 'studyRecord' ? { evaluation: 'revise', feedback: studyFeedbacks[item.id] } : undefined); setMemberNotice(`Solicitação de ${item.name} recusada`); }
  ...
  const saveManagement = async () => {
    ...
    try {
      ...
      if (firebaseEnabled && isClassActivity) { await createClassActivity({ classId: selectedClassId, title: lessonTitle, description: activityDescription, location: eventLocation, dateLabel: eventDate, points: Number(activityPoints) || 20 }); setDirectedActivities(await listDirectedActivities(selectedClassId)); setMemberNotice('Atividade publicada para a sua base'); }
  ...
  return (
    <View>
      ...
      {isEvent && <>
        {districtEvents.map(event => <View key={event.id} style={styles.eventCard}><View style={styles.eventDate}><Text style={styles.eventDay}>◉</Text></View><View style={styles.flex}><Text style={styles.eventTitle}>{event.title}</Text><Text style={styles.eventCopy}>{event.location} · {event.dateLabel}</Text><Text style={styles.challengeStatus}>{event.participantCount ?? 0} participante(s) confirmado(s)</Text></View></View>)}
