  ...
  return (
    <View>
      ...
      {isMembers && <>
        ...
        <View style={styles.formCard}><Text style={styles.manageTitle}>{selectedMemberId ? `Ações para ${displayMembers.find(item => item.id === selectedMemberId)?.name ?? 'o membro'}` : 'Selecione um membro'}</Text><Text style={styles.manageCopy}>Toque em uma pessoa para liberar as ações permitidas.</Text><View style={styles.memberActions}><Pressable disabled={!selectedMemberId} style={[styles.memberActionButton, !selectedMemberId && styles.buttonDisabled]} onPress={() => runMembershipAction('transferLeadership')}><Text style={styles.memberActionText}>⇄ Transferir liderança</Text></Pressable><Pressable style={styles.memberDangerButton} onPress={() => runMembershipAction('revokeDirector')}><Text style={styles.memberDangerText}>Deixar direção</Text></Pressable></View><Pressable disabled={!selectedMemberId || displayMembers.find(item => item.id === selectedMemberId)?.role === 'director'} style={[styles.removeMemberButton, (!selectedMemberId || displayMembers.find(item => item.id === selectedMemberId)?.role === 'director') && styles.buttonDisabled]} onPress={() => runMembershipAction('removeMember')}><Text style={styles.removeMemberText}>Remover adolescente da classe</Text></Pressable></View>
      </>}
