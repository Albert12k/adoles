...
import { createCoordinatorInvite, listCoordinatorInvites, type CoordinatorInvite } from './src/services/coordinatorInvites';
...
function ManagementDetail({ title, role, selectedClassId, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; selectedClassId?: string; onBack: () => void }) {
  ...
  useEffect(() => { if (isCoordinatorInvites) Promise.all([listStructures(), listCoordinatorInvites()]).then(([items, invites]) => { setStructures(items); setCoordinatorInvites(invites); setCoordinatorDistrictId(current => current || items.find(item => item.kind === 'district')?.id || ''); }).catch(() => undefined); }, [isCoordinatorInvites]);
  ...
  return (
    <View>
      ...
      {!isContent && !isQuiz && !isApproval && !isReport && !isEvent && !isStructure && !isCoordinatorInvites && !isRisk && !isChallengeCreation && !isClassActivity && !isPresenceTheme && <>
