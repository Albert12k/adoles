...
import { cancelDistrictEvent, cancelEventAttendance, completeDistrictEvent, confirmEventAttendance, createDistrictEvent, listCurrentDistrictEvents, listDistrictEvents, listMyEventRegistrations, promoteNextWaitlisted, remindEventParticipants, setEventCheckIn, updateDistrictEvent, type DistrictEvent, type MyEventRegistration } from './src/services/events';
  ...
  return (
    <View>
      ...
      {isEvent && <>
        ...
        {districtEvents.some(event => !event.active) && <><Text style={styles.sectionTitle}>Histórico de encontros</Text>{districtEvents.filter(event => !event.active).map(event => <View key={`history-${event.id}`} style={styles.formCard}><Text style={styles.manageTitle}>{event.title}</Text><Text style={styles.manageCopy}>{event.dateLabel} · {event.location}</Text>{event.status === 'completed' ? <><Pill tone="teal">CONCLUÍDO</Pill><Text style={styles.challengeStatus}>{event.checkedInCount ?? 0} presença(s) de {event.participantCount ?? 0} inscrição(ões)</Text></> : <Pill tone="coral">CANCELADO</Pill>}</View>)}</>}
