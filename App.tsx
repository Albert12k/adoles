...
import { createCoordinatorInvite, listCoordinatorAccounts, listCoordinatorInvites, updateCoordinatorAccount, type CoordinatorAccount, type CoordinatorInvite } from './src/services/coordinatorInvites';
...
function ManagementDetail({ title, role, selectedClassId, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; selectedClassId?: string; onBack: () => void }) {
  ...
  useEffect(() => { if (isCoordinatorInvites) Promise.all([listStructures(), listCoordinatorInvites(), listCoordinatorAccounts()]).then(([items, invites, accounts]) => { setStructures(items); setCoordinatorInvites(invites); setCoordinatorAccounts(accounts); setCoordinatorTransfers(Object.fromEntries(accounts.map(item => [item.id, item.districtId]))); setCoordinatorDistrictId(current => current || items.find(item => item.kind === 'district')?.id || ''); }).catch(() => undefined); }, [isCoordinatorInvites]);
