...
function ManagementDetail({ title, role, selectedClassId, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; selectedClassId?: string; onBack: () => void }) {
  ...
  const saveManagement = async () => {
    ...
    try {
      ...
      if (firebaseEnabled && isChallengeCreation) { await submitClassChallenge({ classId: selectedClassId, title: lessonTitle, description: challengeDescription, evidence: challengeEvidence, bonusPoints: Number(challengePoints) || 100 }); setDirectedChallenges(await listDirectedChallenges(selectedClassId)); setMemberNotice('Desafio enviado ao coordenador para validação'); }
