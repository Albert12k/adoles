...
import { archiveWeeklyContent, endQuizNow, getAttendanceProgress, listManagedContent, listManagedQuizzes, listMemberTransferTargets, manageClassMembership, publishContent, publishLatestQuizRanking, publishQuizContent, restoreWeeklyContent, reviewLeadershipItem, sendAttendanceReminder, sendQuizReminder, subscribeQuizParticipation, type AttendanceProgress, type ManagedContent, type ManagedQuiz } from './src/services/management';
import { exportLeadershipReport, loadLeadershipReport, type LeadershipReport } from './src/services/report';
import { selectAndUploadChallengePhoto, selectAndUploadContentPdf, selectAttendancePhoto, uploadSelectedAttendancePhoto } from './src/services/media';
import { markAllNotificationsRead, markNotificationRead, registerPushNotifications } from './src/services/notifications';
...
import { createFlashcard, listMyFlashcards, listPublishedFlashcards, resubmitFlashcard, type Flashcard } from './src/services/flashcards';
import { listApprovedChallenges, listDirectedChallenges, resubmitClassChallenge, submitClassChallenge, type ClassChallenge } from './src/services/challenges';
...
import { getAccountDeletionRequest, getLeadershipSettings, listAccountDeletionRequests, requestAccountDeletion, reviewAccountDeletionRequest, saveLeadershipSettings, type AccountDeletionRequest, type LeadershipSettings } from './src/services/settings';
...
const quizQuestionTemplates: QuizQuestionDraft[] = [
  { type: 'multiple_choice', prompt: '', options: ['', '', '', ''], correctAnswer: 0 },
  { type: 'true_false', prompt: '', options: ['Verdadeiro', 'Falso'], correctAnswer: 0 },
  { type: 'assertion_reason', prompt: '', options: ['As duas são verdadeiras, e II explica I', 'As duas são verdadeiras, mas II não explica I', 'I é verdadeira e II é falsa', 'I é falsa e II é verdadeira'], correctAnswer: 0 },
  { type: 'identify_false', prompt: '', options: ['', '', '', ''], correctAnswer: 0 },
  { type: 'open', prompt: '', options: [], correctAnswer: 'avaliação do diretor' },
];
...
const defaultQuizSchedule = () => { const date = new Date(Date.now() + 24 * 60 * 60 * 1000); return `${String(date.getDate()).padStart(2, '0')}/${String(date.getMonth() + 1).padStart(2, '0')}/${date.getFullYear()} 19:00`; };
const parseQuizSchedule = (value: string) => { const match = value.trim().match(/^(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2})$/); if (!match) return NaN; const [, day, month, year, hour, minute] = match; const date = new Date(Number(year), Number(month) - 1, Number(day), Number(hour), Number(minute), 0, 0); return date.getFullYear() === Number(year) && date.getMonth() === Number(month) - 1 && date.getDate() === Number(day) ? date.getTime() : NaN; };
...
function StudyScreen({ classId, userName }: { classId: string; userName: string }) {
  ...
  const [studySummaries, setStudySummaries] = useState<Record<'lesson' | 'bible' | 'book', string>>({ lesson: '', bible: '', book: '' });
  ...
  const summary = studySummaries[studySource];
  const setSummary = (value: string) => setStudySummaries(current => ({ ...current, [studySource]: value }));
  ...
  const registerStudy = async () => {
    ...
    try {
      await saveStudy({ userId: auth.currentUser.uid, userName, classId, contentId: content?.id ?? 'leitura-biblica-livre', source: studySource, passage: studySource === 'bible' ? biblePassage.trim() : studySource === 'book' ? bookPassage.trim() : undefined, summary: summary.trim(), feedbackVisible: false });
      setCompleted(true);
  ...
  return (
    <View style={styles.pagePad}>
      ...
      {studySource === 'book' && <AuthField label="Capítulo ou páginas do livro" placeholder="Ex.: capítulo 3 · páginas 24–31" value={bookPassage} onChangeText={setBookPassage} />}
...
function ProfileScreen({ name, className, classId, districtId, ageGroup, initialStatus, initialThemeColor, onExit }: { name: string; className: string; classId: string; districtId: string; ageGroup: string; initialStatus: string; initialThemeColor: string; onExit: () => Promise<void> }) {
  ...
  const [flashcardToRevise, setFlashcardToRevise] = useState('');
  ...
  const [selectedPeer, setSelectedPeer] = useState<PublicProfile | null>(null);
  const [peerSearch, setPeerSearch] = useState('');
  const [muralPosts, setMuralPosts] = useState<MuralPost[]>([]);
  const [muralFilter, setMuralFilter] = useState<'all' | 'challenge' | 'achievement'>('all');
  const [notificationFilter, setNotificationFilter] = useState<'all' | 'unread' | 'quiz' | 'event' | 'followup'>('all');
  ...
  const [myDataSummary, setMyDataSummary] = useState<MyDataSummary | null>(null);
  ...
  useEffect(() => { listMyFlashcards().then(setMyFlashcards).catch(() => undefined); }, []);
  ...
  useEffect(() => { if (communityView === 'configuracoes' && !studentSettings) { getLeadershipSettings().then(setStudentSettings).catch(() => setProfileNotice('Não foi possível abrir as configurações.')); getAccountDeletionRequest().then(item => setDeletionPending(item?.status === 'pending')).catch(() => undefined); loadMyDataSummary().then(setMyDataSummary).catch(() => undefined); } }, [communityView, studentSettings]);
  ...
  const react = async (post: MuralPost, emoji: string) => {
    if (!post.id) return;
    try { const reactions = await reactToMuralPost(post.id, emoji); setMuralPosts(items => items.map(item => item.id === post.id ? { ...item, reactions } : item)); } catch { setProfileNotice('Não foi possível registrar a reação.'); }
  };
  const submitFlashcard = async () => {
    ...
    try {
      await createFlashcard({ classId, userName: name, front: flashFront, back: flashBack }); setMyFlashcards(await listMyFlashcards());
  ...
  if (communityView !== 'hub') {
    ...
    return (
      <View style={styles.pagePad}>
        <BackButton onPress={() => selectedPeer ? setSelectedPeer(null) : setCommunityView('hub')} />
        <Text style={styles.pageEyebrow}>{selectedPeer ? 'PERFIL PÚBLICO' : content.eyebrow}</Text><Text style={styles.pageTitle}>{selectedPeer?.name ?? content.title}</Text><Text style={styles.pageIntro}>{selectedPeer ? 'Veja as conquistas públicas de quem caminha com você na mesma turma.' : content.copy}</Text>
        ...
        {communityView === 'mural' && <><View style={styles.scopeWrap}>{([['all', 'Tudo'], ['challenge', 'Desafios'], ['achievement', 'Conquistas']] as const).map(([value, label]) => <Pressable key={value} style={[styles.scopeChip, muralFilter === value && styles.scopeChipActive]} onPress={() => setMuralFilter(value)}><Text style={[styles.scopeChipText, muralFilter === value && styles.scopeChipTextActive]}>{label}</Text></Pressable>)}</View>{muralPosts.filter(post => muralFilter === 'all' || (muralFilter === 'challenge' ? post.type === 'challenge' : post.type !== 'challenge')).length === 0 && <Text style={styles.pageIntro}>Nenhuma publicação nesta categoria.</Text>}{muralPosts.filter(post => muralFilter === 'all' || (muralFilter === 'challenge' ? post.type === 'challenge' : post.type !== 'challenge')).map(post => <View key={post.id} style={styles.feedCard}><Text style={styles.feedEmoji}>{post.icon}</Text><View style={styles.flex}><View style={styles.weekRow}><Text style={styles.manageTitle}>{post.title}</Text>{post.createdAt && <Text style={styles.cardCaption}>{post.createdAt.toLocaleDateString('pt-BR')}</Text>}</View><Text style={styles.manageCopy}>{post.copy}</Text>{post.evidence && <View style={styles.successNotice}><Text style={styles.manageTitle}>Evidência da base</Text><Text style={styles.manageCopy}>{post.evidence}</Text></View>}<View style={styles.reactionRow}>{['♥', '🙌', '⚡', '👏', '✨'].map(emoji => { const count = post.reactions.filter(value => value.endsWith(`:${emoji}`)).length; const selected = Boolean(auth?.currentUser && post.reactions.includes(`${auth.currentUser.uid}:${emoji}`)); return <Pressable key={emoji} style={[styles.reactionButton, selected && styles.scopeChipActive]} onPress={() => react(post, emoji)}><Text style={[styles.reactions, selected && { color: colors.white }]}>{emoji} {count}</Text></Pressable>; })}</View><Text style={styles.cardCaption}>Escolha uma reação · toque novamente para remover</Text></View></View>)}</>}
        {communityView === 'flashcards' && <>
          ...
          {myFlashcards.length > 0 && <><Text style={styles.sectionTitle}>Meus cartões enviados</Text>{myFlashcards.map(card => <View key={`mine_${card.id}`} style={styles.formCard}><View style={styles.weekRow}><Text style={styles.manageTitle}>{card.front}</Text><Pill tone={card.status === 'published' ? 'teal' : card.status === 'rejected' ? 'coral' : 'gold'}>{card.status === 'published' ? 'PUBLICADO' : card.status === 'rejected' ? 'AJUSTAR' : 'EM ANÁLISE'}</Pill></View><Text style={styles.manageCopy}>{card.back}</Text>{card.reviewerFeedback && <Text style={styles.challengeStatus}>Orientação: {card.reviewerFeedback}</Text>}{card.status === 'rejected' && (flashcardToRevise === card.id ? <><AuthField label="Frente corrigida" placeholder="Pergunta" value={flashFront} onChangeText={setFlashFront} /><AuthField label="Resposta corrigida" placeholder="Resposta" value={flashBack} onChangeText={setFlashBack} /><View style={styles.memberActions}><Pressable style={styles.memberActionButton} onPress={async () => { try { await resubmitFlashcard(card, { front: flashFront, back: flashBack }); setMyFlashcards(await listMyFlashcards()); setFlashcardToRevise(''); setFlashFront(''); setFlashBack(''); setFlashNotice('Cartão corrigido e reenviado.'); } catch (error) { setFlashNotice(error instanceof Error ? error.message : 'Não foi possível reenviar.'); } }}><Text style={styles.memberActionText}>Reenviar cartão</Text></Pressable><Pressable style={styles.memberDangerButton} onPress={() => setFlashcardToRevise('')}><Text style={styles.memberDangerText}>Cancelar</Text></Pressable></View></> : <Pressable style={styles.memberActionButton} onPress={() => { setFlashcardToRevise(card.id); setFlashFront(card.front); setFlashBack(card.back); }}><Text style={styles.memberActionText}>Corrigir cartão</Text></Pressable>)}</View>)}</>}
        ...
        {communityView === 'notificacoes' && <><View style={styles.scopeWrap}>{([['all', 'Todas'], ['unread', 'Não lidas'], ['quiz', 'Quiz'], ['event', 'Encontros'], ['followup', 'Acompanhamento']] as const).map(([value, label]) => <Pressable key={value} style={[styles.scopeChip, notificationFilter === value && styles.scopeChipActive]} onPress={() => setNotificationFilter(value)}><Text style={[styles.scopeChipText, notificationFilter === value && styles.scopeChipTextActive]}>{label}</Text></Pressable>)}</View>{live.notifications.some(item => !item.read) && <Pressable style={styles.memberActionButton} onPress={async () => { const count = await markAllNotificationsRead(); setProfileNotice(`${count} notificação(ões) marcada(s) como lida(s).`); }}><Text style={styles.memberActionText}>✓ Marcar todas como lidas</Text></Pressable>}{live.notifications.filter(item => notificationFilter === 'all' || notificationFilter === 'unread' && !item.read || notificationFilter === 'quiz' && item.type.toLowerCase().includes('quiz') || notificationFilter === 'event' && item.type.toLowerCase().includes('event') || notificationFilter === 'followup' && !item.type.toLowerCase().includes('quiz') && !item.type.toLowerCase().includes('event')).length === 0 && <Text style={styles.pageIntro}>Nenhuma notificação nesta categoria.</Text>}{live.notifications.filter(item => notificationFilter === 'all' || notificationFilter === 'unread' && !item.read || notificationFilter === 'quiz' && item.type.toLowerCase().includes('quiz') || notificationFilter === 'event' && item.type.toLowerCase().includes('event') || notificationFilter === 'followup' && !item.type.toLowerCase().includes('quiz') && !item.type.toLowerCase().includes('event')).map(item => <Pressable key={item.id} onPress={() => !item.read && markNotificationRead(item.id)} style={[styles.notificationCard, !item.read && styles.notificationUnread]}><Text style={styles.notificationTag}>{item.type.toUpperCase()}</Text><View style={styles.flex}><View style={styles.weekRow}><Text style={styles.manageTitle}>{item.title}</Text>{item.createdAt && <Text style={styles.cardCaption}>{item.createdAt.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</Text>}</View><Text style={styles.manageCopy}>{item.body}</Text>{!item.read && <Text style={styles.notificationReadHint}>Toque para marcar como lida</Text>}</View>{!item.read && <View style={styles.unreadDot} />}</Pressable>)}{profileNotice !== '' && <Text style={styles.successNotice}>{profileNotice}</Text>}</>}
        ...
        {communityView === 'configuracoes' && studentSettings && <View style={styles.formCard}><Text style={styles.sectionTitle}>Minha cópia de dados</Text><Text style={styles.manageCopy}>Confira e baixe um PDF organizado com os dados vinculados à sua conta.</Text>{myDataSummary && <View style={styles.reportMiniGrid}><Text style={styles.reportMiniItem}>{myDataSummary.studies} estudos</Text><Text style={styles.reportMiniItem}>{myDataSummary.attendance} presenças</Text><Text style={styles.reportMiniItem}>{myDataSummary.quizzes} quizzes</Text><Text style={styles.reportMiniItem}>{myDataSummary.activities + myDataSummary.events} participações</Text></View>}<Pressable disabled={exportingMyData} style={[styles.memberActionButton, exportingMyData && styles.buttonDisabled]} onPress={async () => { setExportingMyData(true); try { await exportMyData(); setProfileNotice('Sua cópia de dados foi gerada.'); } catch (error) { setProfileNotice(error instanceof Error ? error.message : 'Não foi possível gerar sua cópia.'); } finally { setExportingMyData(false); } }}><Text style={styles.memberActionText}>{exportingMyData ? 'Gerando documento...' : '⇩ Baixar meus dados em PDF'}</Text></Pressable></View>}
        ...
        {communityView === 'colegas' && selectedPeer && (() => {
          const latestRank = rankingHistory.flatMap(week => week.entries ?? []).find(entry => entry.userId === selectedPeer.id);
          const bestPeriod = periodHistory.flatMap(period => period.entries).filter(entry => entry.userId === selectedPeer.id).sort((a, b) => a.position - b.position)[0];
          return <>
            <View style={styles.peerProfileHero}>
              <View style={[styles.peerProfileAvatar, { backgroundColor: selectedPeer.themeColor ?? colors.gold }]}><Text style={styles.profileAvatarText}>{selectedPeer.name[0]?.toUpperCase()}</Text></View>
              <Text style={styles.profileName}>{selectedPeer.name}</Text>
              <Text style={styles.profileClass}>{className} · {selectedPeer.ageGroup === 'pre-adolescentes' ? 'Pré-adolescentes' : 'Adolescentes'}</Text>
              <Text style={styles.profileStatus}>“{selectedPeer.status || 'Caminhando com propósito.'}”</Text>
            </View>
            <View style={styles.peerPublicStats}>
              <View style={styles.stat}><Text style={styles.statValue}>{latestRank?.score ?? '—'}</Text><Text style={styles.cardCaption}>pontos publicados</Text></View>
              <View style={styles.stat}><Text style={styles.statValue}>{latestRank?.position ? `${latestRank.position}º` : '—'}</Text><Text style={styles.cardCaption}>ranking semanal</Text></View>
              <View style={styles.stat}><Text style={styles.statValue}>{bestPeriod?.position ? `${bestPeriod.position}º` : '—'}</Text><Text style={styles.cardCaption}>melhor trimestre</Text></View>
            </View>
            <View style={styles.formCard}><Text style={styles.sectionTitle}>Conquistas públicas</Text>{!latestRank && !bestPeriod && <Text style={styles.manageCopy}>As conquistas aparecerão quando a liderança publicar os próximos rankings.</Text>}{latestRank && <View style={styles.publicAchievement}><Text style={styles.communityIcon}>🏆</Text><View style={styles.flex}><Text style={styles.manageTitle}>Ranking semanal publicado</Text><Text style={styles.manageCopy}>{latestRank.position}º lugar · {latestRank.score} pontos</Text></View></View>}{bestPeriod && <View style={styles.publicAchievement}><Text style={styles.communityIcon}>⭐</Text><View style={styles.flex}><Text style={styles.manageTitle}>Destaque do trimestre</Text><Text style={styles.manageCopy}>{bestPeriod.position}º lugar · {bestPeriod.points} pontos</Text></View></View>}</View>
            <View style={styles.peerPrivacyCard}><Text style={styles.manageTitle}>Privacidade protegida</Text><Text style={styles.manageCopy}>Resumos, respostas de quiz, presença, avaliações e dados de contato não são exibidos neste perfil.</Text></View>
          </>;
        })()}
        {communityView === 'colegas' && !selectedPeer && <><TextInput value={peerSearch} onChangeText={setPeerSearch} placeholder="Buscar adolescente pelo nome" placeholderTextColor="#8A9892" style={[styles.authInput, styles.peerSearch]} />{classProfiles.filter(profile => profile.id !== auth?.currentUser?.uid && profile.name.toLowerCase().includes(peerSearch.trim().toLowerCase())).length === 0 && <Text style={styles.pageIntro}>{peerSearch.trim() ? 'Nenhum adolescente encontrado com esse nome.' : 'Os perfis dos colegas aparecerão quando a entrada deles na turma for aprovada.'}</Text>}{classProfiles.filter(profile => profile.id !== auth?.currentUser?.uid && profile.name.toLowerCase().includes(peerSearch.trim().toLowerCase())).map(profile => <Pressable key={profile.id} style={styles.profilePeerCard} onPress={() => setSelectedPeer(profile)}><View style={[styles.rankAvatar, { backgroundColor: profile.themeColor ?? colors.gold }]}><Text style={styles.rankAvatarText}>{profile.name[0]?.toUpperCase()}</Text></View><View style={styles.flex}><Text style={styles.manageTitle}>{profile.name}</Text><Text style={styles.manageCopy}>“{profile.status || 'Caminhando com propósito.'}”</Text></View><Text style={styles.peerOpenHint}>Ver perfil ›</Text></Pressable>)}</>}
  ...
  return (
    <View style={styles.pagePad}>
      ...
      <View style={styles.communityGrid}>
        ...
        ].map(([key, icon, label, bg]) => { const unread = key === 'notificacoes' ? live.notifications.filter(item => !item.read).length : 0; return <Pressable key={key} style={[styles.communityCard, { backgroundColor: bg }]} onPress={() => setCommunityView(key as typeof communityView)}><Text style={styles.communityIcon}>{icon}</Text><Text style={styles.communityLabel}>{label}{unread ? ` · ${unread}` : ''}</Text><Text style={styles.communityLink}>{unread ? 'NOVAS · abrir ›' : 'Abrir ›'}</Text></Pressable>; })}
      </View>
...
function ProtectedStorageImage({ resource }: { resource: string }) {
  const [uri, setUri] = useState(resource.startsWith('supabase://') ? '' : resource);
  useEffect(() => { let active = true; resolvePrivateFileUrl(resource).then(value => { if (active) setUri(value); }).catch(() => undefined); return () => { active = false; }; }, [resource]);
  return uri ? <Image source={{ uri }} style={styles.approvalEvidence} /> : <ActivityIndicator color={colors.tealMedium} />;
}
...
function AuthFlow({ onComplete }: { onComplete: (role: Role) => void }) {
  ...
  const [selectedAgeGroup, setSelectedAgeGroup] = useState<'adolescentes' | 'pre-adolescentes'>('adolescentes');
  ...
  useEffect(() => {
    ...
    getRegistrationOptions().then(options => {
      ...
      setSelectedClass(options.classes.find(item => item.districtId === districtId && item.churchId === churchId && item.ageGroup === 'adolescentes')?.id ?? '');
      ...
      <SafeAreaView style={styles.authSafe}>
        <ScrollView contentContainerStyle={styles.authPage}>
          ...
          {(role === 'diretor' || role === 'professor' || role === 'adolescente') && <View style={styles.scopeSection}>
            ...
            <Text style={[styles.authLabel, { marginTop: 13 }]}>Igreja desejada</Text><View style={styles.scopeWrap}>{registrationOptions.churches.filter(item => item.districtId === selectedDistrict).map(item => <Pressable key={item.id} style={[styles.scopeChip, selectedChurch === item.id && styles.scopeChipActive]} onPress={() => { setSelectedChurch(item.id); setSelectedClass(registrationOptions.classes.find(entry => entry.churchId === item.id && entry.ageGroup === selectedAgeGroup)?.id ?? ''); }}><Text style={[styles.scopeChipText, selectedChurch === item.id && styles.scopeChipTextActive]}>{item.name}</Text></Pressable>)}</View>
            <Text style={[styles.authLabel, { marginTop: 13 }]}>Faixa da turma</Text><View style={styles.scopeWrap}>{([['adolescentes', 'Adolescentes'], ['pre-adolescentes', 'Pré-adolescentes']] as const).map(([value, label]) => <Pressable key={value} style={[styles.scopeChip, selectedAgeGroup === value && styles.scopeChipActive]} onPress={() => { setSelectedAgeGroup(value); setSelectedClass(registrationOptions.classes.find(entry => entry.districtId === selectedDistrict && entry.churchId === selectedChurch && entry.ageGroup === value)?.id ?? ''); }}><Text style={[styles.scopeChipText, selectedAgeGroup === value && styles.scopeChipTextActive]}>{label}</Text></Pressable>)}</View>
            <Text style={[styles.authLabel, { marginTop: 13 }]}>Base desejada</Text><View style={styles.scopeWrap}>{registrationOptions.classes.filter(item => item.districtId === selectedDistrict && item.churchId === selectedChurch && item.ageGroup === selectedAgeGroup).map(item => <Pressable key={item.id} style={[styles.scopeChip, selectedClass === item.id && styles.scopeChipActive]} onPress={() => setSelectedClass(item.id)}><Text style={[styles.scopeChipText, selectedClass === item.id && styles.scopeChipTextActive]}>{item.name} · {item.ageGroup === 'pre-adolescentes' ? 'Pré-adolescentes' : 'Adolescentes'}</Text></Pressable>)}</View>
...
function MetricCard({ value, label, accent, onPress }: { value: string; label: string; accent: string; onPress?: () => void }) {
  return <Pressable disabled={!onPress} onPress={onPress} style={({ pressed }) => [styles.metricCard, { borderTopColor: accent }, pressed && styles.cardPressed]}><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricLabel}>{label}</Text>{onPress && <Text style={styles.metricLink}>Abrir ›</Text>}</Pressable>;
}
...
function ManagementDetail({ title, role, selectedClassId, onBack }: { title: string; role: Exclude<Role, 'adolescente'>; selectedClassId?: string; onBack: () => void }) {
  ...
  const [quizReleaseMode, setQuizReleaseMode] = useState<'now' | 'saturday'>('now');
  const [quizScheduleAt, setQuizScheduleAt] = useState(defaultQuizSchedule);
  ...
  const [approvalConfirmation, setApprovalConfirmation] = useState<{ item: { id: string; name: string }; decision: 'approve' | 'reject' } | null>(null);
  ...
  const [memberTransferTargets, setMemberTransferTargets] = useState<Array<{ id: string; name: string; ageGroup: string }>>([]);
  const [memberTargetClassId, setMemberTargetClassId] = useState('');
  const [memberActionToConfirm, setMemberActionToConfirm] = useState<'removeMember' | 'transferMember' | null>(null);
  ...
  const [challengePhoto, setChallengePhoto] = useState<{ localUri: string; url: string } | null>(null);
  const [challengeAwardPoints, setChallengeAwardPoints] = useState<Record<string, string>>({});
  const [directedChallenges, setDirectedChallenges] = useState<ClassChallenge[]>([]);
  const [challengeToRevise, setChallengeToRevise] = useState('');
  const [challengeReviewNotes, setChallengeReviewNotes] = useState<Record<string, string>>({});
  const [flashcardReviewNote, setFlashcardReviewNote] = useState('');
  ...
  const [followUpOutcomes, setFollowUpOutcomes] = useState<Record<string, EngagementOutcome>>({});
  const [engagementFollowUps, setEngagementFollowUps] = useState<EngagementFollowUp[]>([]);
  ...
  const isDeletionManagement = title === 'Solicitações de exclusão';
  ...
  useEffect(() => { if (isChallengeCreation) listDirectedChallenges(selectedClassId).then(setDirectedChallenges).catch(() => undefined); }, [isChallengeCreation, selectedClassId]);
  useEffect(() => { if (isRisk) listClassEngagement(selectedClassId).then(async result => { setEngagementClassId(result.classId); setEngagementMembers(result.members); setEngagementFollowUps(await listEngagementFollowUps(result.classId)); }).catch(() => undefined); }, [isRisk, selectedClassId]);
  ...
  const approveItem = async (item: { id: string; name: string }, confirmed = false) => {
    if (!confirmed) { setApprovalConfirmation({ item, decision: 'approve' }); return; }
    setApprovalConfirmation(null);
    if (!firebaseEnabled || !approvalType || !item.id) return toggleApproval(item.name);
    setActionError('');
    try { await reviewLeadershipItem(approvalType, item.id, true, approvalType === 'studyRecord' ? { evaluation: studyEvaluations[item.id] ?? 'good', feedback: studyFeedbacks[item.id] } : approvalType === 'attendance' ? { attendanceNote: attendanceNotes[item.id] } : approvalType === 'challenge' ? { feedback: challengeReviewNotes.global, bonusPoints: Number(challengeAwardPoints.global) } : approvalType === 'flashcard' ? { feedback: flashcardReviewNote } : undefined); toggleApproval(item.name); if (approvalType === 'attendance') setAttendanceProgress(await getAttendanceProgress(selectedClassId)); }
    catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível aprovar.'); }
  };
  const rejectItem = async (item: { id: string; name: string }, confirmed = false) => {
    if (!confirmed) { setApprovalConfirmation({ item, decision: 'reject' }); return; }
    setApprovalConfirmation(null);
    if (!firebaseEnabled || !approvalType || !item.id) return;
    setActionError('');
    try { if (approvalType === 'challenge' && !(challengeReviewNotes.global ?? '').trim()) throw new Error('Escreva a orientação antes de devolver o desafio.'); if (approvalType === 'flashcard' && !flashcardReviewNote.trim()) throw new Error('Escreva a orientação antes de devolver o cartão.'); await reviewLeadershipItem(approvalType, item.id, false, approvalType === 'studyRecord' ? { evaluation: 'revise', feedback: studyFeedbacks[item.id] } : approvalType === 'attendance' ? { attendanceNote: attendanceNotes[item.id] } : approvalType === 'challenge' ? { feedback: challengeReviewNotes.global } : approvalType === 'flashcard' ? { feedback: flashcardReviewNote } : undefined); setMemberNotice(`Solicitação de ${item.name} recusada`); if (approvalType === 'attendance') setAttendanceProgress(await getAttendanceProgress(selectedClassId)); }
    catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível recusar.'); }
  };
  useEffect(() => { if (isMembers && classManagement.classId) listMemberTransferTargets(classManagement.classId).then(setMemberTransferTargets).catch(() => setMemberTransferTargets([])); }, [isMembers, classManagement.classId]);
  const runMembershipAction = async (action: 'regenerateCode' | 'removeMember' | 'transferMember' | 'transferLeadership' | 'revokeDirector') => {
    ...
    if ((action === 'removeMember' || action === 'transferMember' || action === 'transferLeadership') && !selectedMemberId) return setActionError('Selecione um membro primeiro.');
    if (action === 'transferMember' && !memberTargetClassId) return setActionError('Escolha a turma de destino.');
    setActionError('');
    try {
      const result = await manageClassMembership({ action, classId: classManagement.classId, targetUserId: selectedMemberId || undefined, targetClassId: memberTargetClassId || undefined });
      setMemberNotice(result.inviteCode ? `Novo código: ${result.inviteCode}` : action === 'transferLeadership' || action === 'revokeDirector' ? 'Solicitação enviada ao coordenador' : action === 'transferMember' ? 'Adolescente transferido para a nova turma' : 'Ação concluída com sucesso');
      if (action === 'transferMember' || action === 'removeMember') { setSelectedMemberId(''); setMemberTargetClassId(''); }
  ...
  const saveManagement = async () => {
    ...
    try {
      ...
      if (firebaseEnabled && isContent) { if (!lessonPdf && !bookPdf) throw new Error('Adicione o PDF da lição, do livro ou ambos.'); await publishContent({ contentId: editingContentId || undefined, classId: selectedClassId, title: lessonTitle, lessonPdfUrl: lessonPdf?.url, bookPdfUrl: bookPdf?.url, week: contentWeek, quarter: contentQuarter, year: new Date().getFullYear() }); setManagedContent(await listManagedContent(selectedClassId)); setEditingContentId(''); setLessonPdf(null); setBookPdf(null); setMemberNotice(editingContentId ? 'Conteúdo semanal atualizado' : 'Conteúdo semanal publicado'); }
      if (firebaseEnabled && isQuiz) { const releaseAt = quizReleaseMode === 'now' ? Date.now() : parseQuizSchedule(quizScheduleAt); if (quizReleaseMode !== 'now' && (!Number.isFinite(releaseAt) || releaseAt <= Date.now())) throw new Error('Informe uma data e um horário futuros no formato DD/MM/AAAA HH:mm.'); await publishQuizContent({ classId: selectedClassId, title: quizTitle.trim(), releaseAt, closesAt: releaseAt + quizDurationDays * 24 * 60 * 60 * 1000, questions: quizQuestions }); setManagedQuizzes(await listManagedQuizzes(selectedClassId)); setMemberNotice(quizReleaseMode === 'now' ? 'Quiz publicado para a base' : `Quiz agendado para ${new Date(releaseAt).toLocaleString('pt-BR')}`); }
  ...
  return (
    <View>
      ...
      {isContent && <View>
        ...
        {managedContent.some(item => !item.archived) && <><Text style={styles.sectionTitle}>Conteúdos disponíveis</Text>{managedContent.filter(item => !item.archived).map(item => <View key={item.id} style={styles.formCard}><View style={styles.weekRow}><View style={styles.flex}><Text style={styles.manageTitle}>{item.title}</Text><Text style={styles.manageCopy}>T{item.quarter} · Semana {item.week} · {item.year} · versão {item.version} · {item.lessonPdfUrl ? 'lição' : ''}{item.lessonPdfUrl && item.bookPdfUrl ? ' + ' : ''}{item.bookPdfUrl ? 'livro' : ''}</Text></View></View><View style={styles.scopeWrap}><Pressable style={styles.scopeChip} onPress={() => { setEditingContentId(item.id); setLessonTitle(item.title); setContentWeek(item.week); setContentQuarter(item.quarter); setLessonPdf(item.lessonPdfUrl ? { name: 'PDF atual da lição', url: item.lessonPdfUrl } : null); setBookPdf(item.bookPdfUrl ? { name: 'PDF atual do livro', url: item.bookPdfUrl } : null); }}><Text style={styles.scopeChipText}>Editar ou substituir PDFs</Text></Pressable><Pressable style={styles.scopeChip} onPress={async () => { try { await archiveWeeklyContent(item.id); setManagedContent(await listManagedContent(selectedClassId)); setMemberNotice('Conteúdo arquivado e retirado dos estudos'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível arquivar.'); } }}><Text style={[styles.scopeChipText, { color: colors.coral }]}>Arquivar</Text></Pressable></View></View>)}</>}
        {managedContent.some(item => item.archived) && <><Text style={styles.sectionTitle}>Histórico arquivado</Text>{managedContent.filter(item => item.archived).map(item => <View key={`archived-${item.id}`} style={styles.formCard}><View style={styles.weekRow}><View style={styles.flex}><Text style={styles.manageTitle}>{item.title}</Text><Text style={styles.manageCopy}>T{item.quarter} · Semana {item.week} · {item.year} · versão {item.version}</Text></View><Pill tone="coral">ARQUIVADO</Pill></View><Pressable style={styles.memberActionButton} onPress={async () => { try { await restoreWeeklyContent(item.id); setManagedContent(await listManagedContent(selectedClassId)); setMemberNotice('Conteúdo restaurado para os adolescentes'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível restaurar.'); } }}><Text style={styles.memberActionText}>Restaurar publicação</Text></Pressable></View>)}</>}
      </View>}
      {isQuiz && <>
        ...
        <View style={styles.formCard}><AuthField label="Título do quiz" placeholder="Ex.: Jornada de Josué" value={quizTitle} onChangeText={setQuizTitle} /></View>
        <View style={styles.formCard}><Text style={styles.manageTitle}>Jornada com {quizQuestions.length} fases</Text><Text style={styles.manageCopy}>{quizQuestions.length ? 'Misture formatos para manter o quiz dinâmico, reflexivo e divertido.' : 'O quiz está vazio. Escolha abaixo o primeiro formato e escreva suas próprias perguntas.'}</Text></View>
        ...
        <View style={styles.formCard}><Text style={styles.sectionTitle}>Publicação do quiz</Text><Text style={styles.manageCopy}>Depois de terminar as perguntas, escolha se deseja publicar imediatamente ou definir o dia e horário.</Text><Text style={[styles.authLabel, { marginTop: 16 }]}>Como publicar?</Text><View style={styles.scopeWrap}>{([['now', 'Publicar agora'], ['saturday', 'Agendar dia e horário']] as const).map(([value, label]) => <Pressable key={value} style={[styles.scopeChip, quizReleaseMode === value && styles.scopeChipActive]} onPress={() => setQuizReleaseMode(value)}><Text style={[styles.scopeChipText, quizReleaseMode === value && styles.scopeChipTextActive]}>{label}</Text></Pressable>)}</View>{quizReleaseMode === 'saturday' && <AuthField label="Data e horário" placeholder="DD/MM/AAAA HH:mm" value={quizScheduleAt} onChangeText={setQuizScheduleAt} />}<Text style={[styles.authLabel, { marginTop: 14 }]}>Prazo para responder</Text><View style={styles.scopeWrap}>{[3, 7, 14].map(days => <Pressable key={days} style={[styles.scopeChip, quizDurationDays === days && styles.scopeChipActive]} onPress={() => setQuizDurationDays(days)}><Text style={[styles.scopeChipText, quizDurationDays === days && styles.scopeChipTextActive]}>{days} dias</Text></Pressable>)}</View></View>
        <View style={styles.inviteCodeCard}><Text style={styles.authEyebrow}>RESUMO DA PUBLICAÇÃO</Text><Text style={styles.manageCopy}>{quizTitle || 'Quiz sem título'} · {quizQuestions.length} fase(s)</Text><Text style={[styles.cardCaption, { color: colors.white }]}>{quizReleaseMode === 'now' ? 'Liberação imediata' : `Agendado para ${quizScheduleAt}`} · encerra após {quizDurationDays} dias</Text></View>
      </>}
      ...
      {isApproval && approvalType === 'challenge' && <View style={styles.formCard}><Text style={styles.sectionTitle}>Validação do coordenador</Text><Text style={styles.manageCopy}>Confira a foto, defina os pontos e registre sua orientação antes de aprovar.</Text>{displayApprovals.map(item => item.evidenceUrl ? <View key={`evidence_${item.id}`}><Text style={styles.authLabel}>{item.name.toUpperCase()}</Text><ProtectedStorageImage resource={item.evidenceUrl} /></View> : null)}<AuthField label="Pontos extras" placeholder="Entre 10 e 500" value={challengeAwardPoints.global ?? ''} onChangeText={text => setChallengeAwardPoints({ global: text.replace(/\D/g, '') })} /><TextInput multiline value={challengeReviewNotes.global ?? ''} onChangeText={text => setChallengeReviewNotes({ global: text })} placeholder="Explique a aprovação ou os ajustes necessários" placeholderTextColor="#8A9892" style={[styles.authInput, styles.textArea]} /></View>}
      {isApproval && approvalType === 'flashcard' && <View style={styles.formCard}><Text style={styles.sectionTitle}>Orientação para os cartões</Text><Text style={styles.manageCopy}>Ao devolver um cartão, explique de forma simples o que precisa ser corrigido.</Text><TextInput multiline value={flashcardReviewNote} onChangeText={setFlashcardReviewNote} placeholder="Ex.: deixe a pergunta mais curta" placeholderTextColor="#8A9892" style={[styles.authInput, styles.textArea]} /></View>}
      {isChallengeCreation && <View style={styles.formCard}><AuthField label="Nome do desafio" placeholder="Ex.: Corrente do bem" value={lessonTitle} onChangeText={setLessonTitle} /><AuthField label="Missão da turma" placeholder="Explique o que foi realizado" value={challengeDescription} onChangeText={setChallengeDescription} /><AuthField label="Relato da evidência" placeholder="Conte como a turma concluiu a missão" value={challengeEvidence} onChangeText={setChallengeEvidence} /><Text style={styles.authLabel}>FOTO DO DESAFIO CUMPRIDO</Text><Pressable style={styles.uploadBox} onPress={async () => { try { const photo = await selectAndUploadChallengePhoto(selectedClassId); if (photo) setChallengePhoto(photo); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível enviar a foto.'); } }}>{challengePhoto ? <Image source={{ uri: challengePhoto.localUri }} style={styles.attendancePreview} /> : <><Text style={styles.uploadIcon}>＋</Text><Text style={styles.uploadTitle}>Adicionar foto da evidência</Text><Text style={styles.uploadCopy}>A imagem será conferida pelo coordenador</Text></>}</Pressable><Text style={styles.manageCopy}>O coordenador distrital analisará a foto e será o único responsável por definir os pontos extras.</Text></View>}
      {isChallengeCreation && directedChallenges.length > 0 && <><Text style={styles.sectionTitle}>Histórico de desafios</Text>{directedChallenges.map(challenge => <View key={challenge.id} style={styles.formCard}><View style={styles.weekRow}><Text style={styles.manageTitle}>{challenge.title}</Text><Pill tone={challenge.status === 'approved' ? 'teal' : challenge.status === 'rejected' ? 'coral' : 'gold'}>{challenge.status === 'approved' ? 'APROVADO' : challenge.status === 'rejected' ? 'AJUSTAR' : 'EM ANÁLISE'}</Pill></View><Text style={styles.manageCopy}>{challenge.description}</Text><Text style={styles.challengeStatus}>+{challenge.bonusPoints} pontos · {challenge.resubmissionCount ?? 0} reenvio(s)</Text>{challenge.reviewerFeedback && <View style={styles.warningCard}><Text style={styles.manageTitle}>Orientação do coordenador</Text><Text style={styles.manageCopy}>{challenge.reviewerFeedback}</Text></View>}{challenge.status === 'rejected' && (challengeToRevise === challenge.id ? <><AuthField label="Missão corrigida" placeholder="Ajuste a descrição" value={challengeDescription} onChangeText={setChallengeDescription} /><AuthField label="Evidência corrigida" placeholder="Ajuste a evidência" value={challengeEvidence} onChangeText={setChallengeEvidence} /><View style={styles.memberActions}><Pressable style={styles.memberActionButton} onPress={async () => { try { await resubmitClassChallenge(challenge, { description: challengeDescription, evidence: challengeEvidence }); setDirectedChallenges(await listDirectedChallenges(selectedClassId)); setChallengeToRevise(''); setMemberNotice('Desafio corrigido e reenviado'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível reenviar.'); } }}><Text style={styles.memberActionText}>Reenviar para análise</Text></Pressable><Pressable style={styles.memberDangerButton} onPress={() => setChallengeToRevise('')}><Text style={styles.memberDangerText}>Cancelar</Text></Pressable></View></> : <Pressable style={styles.memberActionButton} onPress={() => { setChallengeToRevise(challenge.id); setChallengeDescription(challenge.description); setChallengeEvidence(challenge.evidence); }}><Text style={styles.memberActionText}>Corrigir desafio</Text></Pressable>)}</View>)}</>}
      ...
      {isRisk && <><View style={styles.engagementSummary}><View style={styles.stat}><Text style={styles.statValue}>{engagementMembers.length}</Text><Text style={styles.cardCaption}>acompanhados</Text></View><View style={styles.stat}><Text style={styles.statValue}>{engagementMembers.filter(item => item.risk === 'high').length}</Text><Text style={styles.cardCaption}>risco alto</Text></View><View style={styles.stat}><Text style={styles.statValue}>{Math.round(engagementMembers.reduce((sum, item) => sum + item.engagement, 0) / Math.max(1, engagementMembers.length))}%</Text><Text style={styles.cardCaption}>engajamento</Text></View></View><View style={styles.filterRow}>{(['all', 'high', 'medium', 'regular'] as const).map(filter => <Pressable key={filter} onPress={() => setRiskFilter(filter)} style={[styles.filterChip, riskFilter === filter && styles.filterChipActive]}><Text style={[styles.filterChipText, riskFilter === filter && styles.filterChipTextActive]}>{({ all: 'Todos', high: 'Alto', medium: 'Médio', regular: 'Regular' } as const)[filter]}</Text></Pressable>)}</View>{engagementMembers.filter(item => riskFilter === 'all' || item.risk === riskFilter).map(member => { const color = member.risk === 'high' ? colors.coral : member.risk === 'medium' ? colors.gold : colors.tealMedium; const outcome = followUpOutcomes[member.userId] ?? 'contacted'; return <View key={member.userId} style={styles.formCard}><View style={styles.riskCard}><View style={[styles.riskLine, { backgroundColor: color }]} /><View style={styles.rankAvatar}><Text style={styles.rankAvatarText}>{member.name[0]}</Text></View><View style={styles.flex}><Text style={styles.manageTitle}>{member.name}</Text><Text style={styles.manageCopy}>{member.daysInactive === 999 ? 'Ainda sem participação registrada' : `${member.daysInactive} dia(s) sem atividade`} · {member.engagement}% engajamento</Text><Text style={styles.riskMetrics}>{member.studies} estudos · {member.attendance} presenças · {member.quizzes} quizzes · {member.activities} atividades</Text>{member.lastContactAt && <Text style={styles.challengeStatus}>Último contato: {member.lastContactAt.toLocaleDateString('pt-BR')}</Text>}</View><Text style={[styles.riskLevel, { color }]}>{member.risk === 'high' ? 'ALTO' : member.risk === 'medium' ? 'MÉDIO' : 'REGULAR'}</Text></View><View style={styles.scopeWrap}>{([['contacted', 'Conversei'], ['no_response', 'Sem resposta'], ['reengaged', 'Retornou']] as const).map(([value, label]) => <Pressable key={value} style={[styles.scopeChip, outcome === value && styles.scopeChipActive]} onPress={() => setFollowUpOutcomes(current => ({ ...current, [member.userId]: value }))}><Text style={[styles.scopeChipText, outcome === value && styles.scopeChipTextActive]}>{label}</Text></Pressable>)}</View><TextInput multiline value={followUpNotes[member.userId] ?? ''} onChangeText={text => setFollowUpNotes(current => ({ ...current, [member.userId]: text }))} placeholder="Observação privada sobre o acompanhamento" placeholderTextColor="#8A9892" style={[styles.authInput, styles.textArea]} /><Pressable style={styles.memberActionButton} onPress={async () => { try { await recordEngagementFollowUp(engagementClassId, member, { outcome, note: followUpNotes[member.userId] ?? '' }); setEngagementFollowUps(await listEngagementFollowUps(engagementClassId)); setEngagementMembers((await listClassEngagement(engagementClassId)).members); setFollowUpNotes(current => ({ ...current, [member.userId]: '' })); setMemberNotice(`Acompanhamento de ${member.name} registrado`); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível registrar.'); } }}><Text style={styles.memberActionText}>Registrar acompanhamento</Text></Pressable></View>; })}{engagementMembers.length === 0 && <Text style={styles.pageIntro}>Nenhum adolescente ativo foi encontrado nesta base.</Text>}{engagementFollowUps.length > 0 && <><Text style={styles.sectionTitle}>Histórico de contatos</Text>{engagementFollowUps.slice(0, 30).map(item => <View key={item.id} style={styles.memberRow}><View style={styles.flex}><Text style={styles.manageTitle}>{item.userName} · {item.outcome === 'reengaged' ? 'Retornou' : item.outcome === 'no_response' ? 'Sem resposta' : 'Conversa realizada'}</Text><Text style={styles.manageCopy}>{item.note || 'Sem observação'}{item.lastContactAt ? ` · ${item.lastContactAt.toLocaleDateString('pt-BR')}` : ''}</Text></View></View>)}</>}</>}
      ...
      {isDeletionManagement && <><View style={styles.warningCard}><Text style={styles.manageTitle}>Processo protegido</Text><Text style={styles.manageCopy}>A aprovação suspende o acesso da conta. A remoção definitiva dos dados deve ocorrer somente após a conferência administrativa.</Text></View>{deletionRequests.length === 0 && <Text style={styles.pageIntro}>Nenhuma solicitação de exclusão foi registrada.</Text>}{deletionRequests.map(request => <View key={request.id} style={styles.formCard}><View style={styles.weekRow}><View style={styles.flex}><Text style={styles.manageTitle}>{request.name || 'Conta sem nome'}</Text><Text style={styles.manageCopy}>{request.email} · {request.role}</Text></View><Pill tone={request.status === 'pending' ? 'gold' : request.status === 'approved' ? 'coral' : 'teal'}>{request.status === 'pending' ? 'PENDENTE' : request.status === 'approved' ? 'ACESSO SUSPENSO' : 'RECUSADA'}</Pill></View><Text style={styles.challengeStatus}>Solicitada em {request.requestedAt?.toLocaleDateString('pt-BR') ?? 'data não informada'}</Text><Text style={styles.manageCopy}>Motivo: {request.reason || 'Não informado'}</Text>{request.status === 'pending' ? <><TextInput multiline value={deletionReviewNotes[request.id] ?? ''} onChangeText={text => setDeletionReviewNotes(current => ({ ...current, [request.id]: text }))} placeholder="Observação administrativa" placeholderTextColor="#8A9892" style={[styles.authInput, styles.textArea]} />{deletionToApprove === request.id ? <View style={styles.warningCard}><Text style={styles.manageTitle}>{adminDeletionSeconds > 0 ? `Aguarde ${adminDeletionSeconds} segundos` : 'Suspender esta conta?'}</Text><Text style={styles.manageCopy}>O usuário perderá o acesso imediatamente após a confirmação.</Text><View style={styles.memberActions}><Pressable disabled={adminDeletionSeconds > 0} style={[styles.memberDangerButton, adminDeletionSeconds > 0 && styles.buttonDisabled]} onPress={async () => { try { await reviewAccountDeletionRequest(request, true, deletionReviewNotes[request.id] ?? ''); setDeletionRequests(await listAccountDeletionRequests()); setDeletionToApprove(''); setAdminDeletionSeconds(10); setMemberNotice('Solicitação aprovada e acesso suspenso'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível aprovar.'); } }}><Text style={styles.memberDangerText}>{adminDeletionSeconds > 0 ? 'Aguarde' : 'Confirmar suspensão'}</Text></Pressable><Pressable style={styles.memberActionButton} onPress={() => { setDeletionToApprove(''); setAdminDeletionSeconds(10); }}><Text style={styles.memberActionText}>Cancelar</Text></Pressable></View></View> : <View style={styles.memberActions}><Pressable style={styles.memberDangerButton} onPress={() => { setDeletionToApprove(request.id); setAdminDeletionSeconds(10); }}><Text style={styles.memberDangerText}>Aprovar exclusão</Text></Pressable><Pressable style={styles.memberActionButton} onPress={async () => { try { await reviewAccountDeletionRequest(request, false, deletionReviewNotes[request.id] ?? 'Solicitação recusada após análise.'); setDeletionRequests(await listAccountDeletionRequests()); setMemberNotice('Solicitação recusada e conta mantida ativa'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível recusar.'); } }}><Text style={styles.memberActionText}>Recusar solicitação</Text></Pressable></View>}</> : <Text style={styles.manageCopy}>Analisada em {request.reviewedAt?.toLocaleDateString('pt-BR') ?? 'data não informada'} · {request.reviewNote || 'Sem observação'}</Text>}</View>)}</>}
      {isRisk && engagementFollowUps.some(item => !item.resolved) && <><Text style={styles.sectionTitle}>Próximos retornos</Text>{engagementFollowUps.filter(item => !item.resolved).sort((a, b) => Number(a.nextFollowUpAt?.getTime() ?? 0) - Number(b.nextFollowUpAt?.getTime() ?? 0)).map(item => { const overdue = Boolean(item.nextFollowUpAt && item.nextFollowUpAt.getTime() < Date.now()); return <View key={`pending_${item.id}`} style={[styles.formCard, overdue && styles.warningCard]}><View style={styles.weekRow}><View style={styles.flex}><Text style={styles.manageTitle}>{item.userName}</Text><Text style={styles.manageCopy}>{overdue ? 'Retorno atrasado' : 'Próximo retorno'} · {item.nextFollowUpAt?.toLocaleDateString('pt-BR') ?? 'sem data'}</Text></View><Pill tone={overdue ? 'coral' : 'gold'}>{overdue ? 'ATRASADO' : 'AGENDADO'}</Pill></View><Text style={styles.manageCopy}>{item.note || 'Sem observação no contato anterior.'}</Text><Pressable style={styles.memberActionButton} onPress={async () => { try { await resolveEngagementFollowUp(item); setEngagementFollowUps(await listEngagementFollowUps(engagementClassId)); setMemberNotice(`Retorno de ${item.userName} concluído`); } catch (error) { setActionError(error instanceof Error ? error.message : 'Não foi possível concluir o retorno.'); } }}><Text style={styles.memberActionText}>Marcar retorno como concluído</Text></Pressable></View>; })}</>}
      {isApproval && approvalConfirmation && <View style={styles.confirmationCard}><Text style={styles.pageEyebrow}>{approvalConfirmation.decision === 'approve' ? 'CONFIRMAR APROVAÇÃO' : 'CONFIRMAR RECUSA'}</Text><Text style={styles.sectionTitle}>{approvalConfirmation.decision === 'approve' ? `Aprovar ${approvalConfirmation.item.name}?` : `Recusar a solicitação de ${approvalConfirmation.item.name}?`}</Text><Text style={styles.manageCopy}>Confira o nome antes de continuar. Nenhuma outra solicitação será alterada.</Text><View style={styles.memberActions}><Pressable style={approvalConfirmation.decision === 'approve' ? styles.memberActionButton : styles.memberDangerButton} onPress={() => approvalConfirmation.decision === 'approve' ? approveItem(approvalConfirmation.item, true) : rejectItem(approvalConfirmation.item, true)}><Text style={approvalConfirmation.decision === 'approve' ? styles.memberActionText : styles.memberDangerText}>{approvalConfirmation.decision === 'approve' ? 'Sim, aprovar esta pessoa' : 'Sim, recusar esta pessoa'}</Text></Pressable><Pressable style={styles.memberActionButton} onPress={() => setApprovalConfirmation(null)}><Text style={styles.memberActionText}>Cancelar</Text></Pressable></View></View>}
      {memberNotice !== '' && <Text style={styles.successNotice}>✓ {memberNotice}</Text>}
      {actionError !== '' && <Text style={styles.authError}>{actionError}</Text>}
      {approvalType === 'classJoinRequest' && <View style={styles.inviteCodeCard}><Text style={styles.authEyebrow}>CÓDIGO DE ENTRADA</Text><Text style={styles.inviteCode}>{classManagement.inviteCode || 'Ainda não gerado'}</Text><Text style={styles.cardCaption}>Use este código somente para aprovar novas entradas.</Text><Pressable style={styles.copyButton} onPress={() => runMembershipAction('regenerateCode')}><Text style={styles.copyButtonText}>Gerar novo código</Text></Pressable></View>}
      {isMembers && <>
        {classManagement.error !== '' && <Text style={styles.authError}>{classManagement.error}</Text>}
        {displayMembers.length === 0 && <View style={styles.formCard}><Text style={styles.manageTitle}>Nenhum membro ativo</Text><Text style={styles.manageCopy}>Os adolescentes aprovados aparecerão aqui automaticamente.</Text></View>}
        ...
        <View style={styles.formCard}><Text style={styles.manageTitle}>{selectedMemberId ? `Ações para ${displayMembers.find(item => item.id === selectedMemberId)?.name ?? 'o membro'}` : 'Selecione um membro'}</Text><Text style={styles.manageCopy}>O diretor pode organizar a turma, mas somente o administrador pode excluir definitivamente uma conta.</Text>{selectedMemberId && memberTransferTargets.length > 0 && <><Text style={[styles.authLabel, { marginTop: 14 }]}>TRANSFERIR PARA OUTRA TURMA</Text><View style={styles.scopeWrap}>{memberTransferTargets.map(item => <Pressable key={item.id} style={[styles.scopeChip, memberTargetClassId === item.id && styles.scopeChipActive]} onPress={() => setMemberTargetClassId(item.id)}><Text style={[styles.scopeChipText, memberTargetClassId === item.id && styles.scopeChipTextActive]}>{item.name} · {item.ageGroup === 'pre-adolescentes' ? 'Pré-adolescentes' : 'Adolescentes'}</Text></Pressable>)}</View><Pressable disabled={!memberTargetClassId} style={[styles.memberActionButton, !memberTargetClassId && styles.buttonDisabled]} onPress={() => setMemberActionToConfirm('transferMember')}><Text style={styles.memberActionText}>⇄ Transferir adolescente</Text></Pressable></>}{selectedMemberId && <><Pressable style={[styles.memberActionButton, { marginTop: 10 }]} onPress={() => runMembershipAction('transferLeadership')}><Text style={styles.memberActionText}>♙ Indicar para direção</Text></Pressable><Pressable style={styles.removeMemberButton} onPress={() => setMemberActionToConfirm('removeMember')}><Text style={styles.removeMemberText}>Remover adolescente desta turma</Text></Pressable></>}{memberActionToConfirm && <View style={styles.confirmationCard}><Text style={styles.manageTitle}>{memberActionToConfirm === 'transferMember' ? 'Confirmar transferência?' : 'Confirmar remoção da turma?'}</Text><Text style={styles.manageCopy}>{memberActionToConfirm === 'transferMember' ? 'O adolescente passará a acessar o conteúdo e as atividades da turma de destino.' : 'A conta continuará existindo, mas perderá o acesso a esta turma.'}</Text><View style={styles.memberActions}><Pressable style={styles.memberDangerButton} onPress={async () => { const action = memberActionToConfirm; setMemberActionToConfirm(null); await runMembershipAction(action); }}><Text style={styles.memberDangerText}>Confirmar</Text></Pressable><Pressable style={styles.memberActionButton} onPress={() => setMemberActionToConfirm(null)}><Text style={styles.memberActionText}>Cancelar</Text></Pressable></View></View>}</View>
      </>}
      {!isApproval && !isReport && !isStructure && !isCoordinatorInvites && !isPeriodClosure && !isLeadershipHistory && !isDeletionManagement && <Pressable style={[styles.authPrimary, saved && styles.buttonDone]} onPress={saveManagement}><Text style={styles.authPrimaryText}>{saved ? '✓ Alterações salvas' : isPresenceTheme ? 'Aplicar tema à base' : isClassActivity ? editingActivityId ? 'Salvar edição' : 'Publicar atividade' : isChallengeCreation ? 'Enviar para validação' : isQuizRanking ? 'Publicar notas e ranking' : isQuiz ? quizReleaseMode === 'now' ? 'Publicar quiz agora' : 'Agendar quiz' : isContent ? editingContentId ? 'Salvar nova versão' : 'Publicar conteúdo' : isEvent ? 'Salvar encontro' : 'Salvar alterações'}</Text></Pressable>}
    </View>
  );
}
...
function ManagementApp({ role, onExit }: { role: Exclude<Role, 'adolescente'>; onExit: () => Promise<void> }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <StatusBar style="light" />
      <View style={[styles.shell, isWide && styles.managementShellWide]}>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.managementContent, isWide && styles.managementContentWide]} showsVerticalScrollIndicator={false}>
          {section === 'painel' && <>
            ...
            <View style={styles.metricsGrid}>{metrics.map(([value, label, accent], index) => <MetricCard key={label} value={value} label={label} accent={accent} onPress={role === 'diretor' ? () => { setSection('gestao'); setSelectedAction(index === 0 ? 'Gerenciar membros' : index === 1 ? 'Aprovar entradas' : 'Relatório da base'); } : undefined} />)}</View>
...
const styles = StyleSheet.create({
  ...
  managementHero: { backgroundColor: colors.teal, padding: 22, paddingBottom: 28, borderBottomLeftRadius: 28, borderBottomRightRadius: 28 }, managementHeroWide: { margin: 20, marginBottom: 0, borderRadius: 28, paddingHorizontal: 32 }, managementTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, managementGreeting: { color: colors.white, fontSize: 26, fontWeight: '900', marginTop: 3 }, managementScope: { color: '#BFD2CD', fontSize: 13, marginTop: 18 }, classSelector: { alignSelf: 'flex-start', backgroundColor: colors.tealMedium, borderWidth: 1, borderColor: '#43736E', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 9, marginTop: 13 }, classSelectorText: { color: colors.white, fontSize: 11, fontWeight: '800' }, managementContent: { padding: 20, paddingBottom: 30 }, managementContentWide: { width: '100%', maxWidth: 1040, alignSelf: 'center', paddingHorizontal: 32 }, classSwitcher: { backgroundColor: colors.white, borderRadius: 17, padding: 12, marginBottom: 15 }, classSwitchButton: { minWidth: 145, borderRadius: 13, backgroundColor: colors.sage, borderWidth: 2, borderColor: 'transparent', padding: 11, marginRight: 8 }, classSwitchButtonActive: { backgroundColor: '#FFF3DB', borderColor: colors.gold }, classSwitchName: { color: colors.ink, fontSize: 11, fontWeight: '800' }, classSwitchNameActive: { color: colors.teal, fontWeight: '900' }, classSwitchGroup: { color: colors.muted, fontSize: 8, marginTop: 4 },
  metricsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, marginTop: 13, marginBottom: 18 }, metricCard: { flex: 1, minWidth: 135, minHeight: 105, backgroundColor: colors.white, borderRadius: 16, borderTopWidth: 4, padding: 14, justifyContent: 'center' }, metricValue: { color: colors.teal, fontSize: 23, fontWeight: '900' }, metricLabel: { color: colors.muted, fontSize: 9, lineHeight: 13, marginTop: 4 }, metricLink: { color: colors.coral, fontSize: 9, fontWeight: '900', marginTop: 8 }, cardPressed: { opacity: 0.72, transform: [{ scale: 0.99 }] },
  ...
  shell: { flex: 1, width: '100%', maxWidth: 520, alignSelf: 'center', backgroundColor: colors.sage }, managementShellWide: { maxWidth: 1180, borderLeftWidth: 1, borderRightWidth: 1, borderColor: colors.line },
  ...
  profileTop: { alignItems: 'center', paddingVertical: 15 }, profileAvatar: { width: 88, height: 88, borderRadius: 44, backgroundColor: colors.gold, borderWidth: 5, borderColor: '#F7E3BA', alignItems: 'center', justifyContent: 'center' }, profileAvatarText: { color: colors.teal, fontSize: 36, fontWeight: '900' }, profileName: { color: colors.ink, fontSize: 24, fontWeight: '900', marginTop: 12 }, profileClass: { color: colors.coral, fontWeight: '800', marginTop: 4, fontSize: 12 }, profileStatus: { color: colors.muted, marginTop: 10, fontStyle: 'italic' }, colorChoices: { flexDirection: 'row', gap: 12, marginBottom: 15 }, colorChoice: { width: 34, height: 34, borderRadius: 17 }, colorChoiceActive: { borderWidth: 4, borderColor: colors.ink }, profilePeerCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.white, borderRadius: 17, padding: 14, marginBottom: 9, borderWidth: 1, borderColor: colors.line }, peerSearch: { marginBottom: 14 }, peerOpenHint: { color: colors.coral, fontSize: 10, fontWeight: '900' }, peerProfileHero: { alignItems: 'center', backgroundColor: colors.white, borderRadius: 24, padding: 24, marginBottom: 12 }, peerProfileAvatar: { width: 96, height: 96, borderRadius: 48, borderWidth: 5, borderColor: '#F7E3BA', alignItems: 'center', justifyContent: 'center' }, peerPublicStats: { flexDirection: 'row', backgroundColor: colors.white, borderRadius: 20, paddingVertical: 18, marginBottom: 12 }, publicAchievement: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.sage, borderRadius: 16, padding: 13, marginTop: 10 }, peerPrivacyCard: { backgroundColor: '#FFF3DB', borderRadius: 17, padding: 15, marginBottom: 12, borderLeftWidth: 4, borderLeftColor: colors.gold }, themeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 16 }, themeCard: { width: '47%', minHeight: 105, borderRadius: 16, backgroundColor: colors.sage, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'transparent' }, themeCardActive: { backgroundColor: '#FFF3DB', borderColor: colors.gold }, themeIcon: { fontSize: 30, marginRight: 10 }, autoScenarioCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, backgroundColor: colors.sage, padding: 14, borderWidth: 2, borderColor: 'transparent', marginTop: 15 }, scenarioPreview: { minHeight: 240, borderRadius: 20, padding: 18, alignItems: 'center', justifyContent: 'center', marginTop: 8 }, scenarioPreviewIcon: { fontSize: 48 }, scenarioTrail: { width: '100%', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 6, marginVertical: 18 }, scenarioStep: { width: 26, height: 26, borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, scenarioStepText: { color: colors.teal, fontSize: 8, fontWeight: '900' }, scenarioNavigation: { flexDirection: 'row', gap: 8, marginTop: 10 }, worldChips: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, worldChip: { backgroundColor: colors.sage, borderRadius: 14, paddingHorizontal: 10, paddingVertical: 8 }, worldChipText: { color: colors.teal, fontSize: 9, fontWeight: '800' },
